# Contributing to Satima Board

The following is a set of guidelines to contribute to Satima Board and its repositories, which are hosted on the [0x6flab Organization](https://github.com/0x6flab) on GitHub.
...
Before raising a new issue, check [our issue list](https://github.com/0x6flab/satima-board/issues) to determine if it already contains the problem that you are facing.
...
```bash
git clone https://github.com/<your-username>/satima-board.git
cd satima-board
git remote add upstream https://github.com/0x6flab/satima-board.git
```
...
Create a new topic branch from `main` using the naming convention `SB-[issue-number]`
to help us keep track of your contribution scope:

```bash
git checkout -b SB-[issue-number]
```
...
```bash
git push origin SB-[issue-number]
```
